...
export function Dashboard() {
  ...
  const { netBalance, incomeThisMonth, expensesThisMonth } = useMemo(() => {
    const now = new Date();
    ...
    state.transactions.forEach(t => {
      ...
      if (t.type === 'income') {
        net += t.amount;
        if (isThisMonth) incMonth += t.amount;
      } else {
        net -= t.amount;
    ...
    return { netBalance: net, incomeThisMonth: incMonth, expensesThisMonth: expMonth };
  }, [state.transactions]);
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="glass p-5 md:p-6 rounded-2xl relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300 cursor-default">
          ...
          <div className="flex justify-between items-start mb-4">
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Income (This Month)</p>
            <div className="p-2 rounded-lg bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400">
              <ArrowUpRight size={20} />
            </div>
          </div>
          <h3 className="text-2xl md:text-3xl font-mono font-semibold text-emerald-600 dark:text-emerald-400 tracking-tight">{currency}{incomeThisMonth.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
        </div>

        <div className="glass p-5 md:p-6 rounded-2xl relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300 cursor-default">
          <div className="absolute inset-0 bg-gradient-to-br from-rose-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
          <div className="flex justify-between items-start mb-4">
            <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Expenses (This Month)</p>
            <div className="p-2 rounded-lg bg-rose-100 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400">
              <ArrowDownRight size={20} />
            </div>
          </div>
          <h3 className="text-2xl md:text-3xl font-mono font-semibold text-rose-600 dark:text-rose-400 tracking-tight">{currency}{expensesThisMonth.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
        </div>
      </div>
         ...
         <div className="xl:col-span-2 space-y-6 md:space-y-8">
           <div className="glass p-5 md:p-6 rounded-2xl h-[350px] md:h-[400px] flex flex-col relative overflow-hidden">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-semibold tracking-wide text-gray-900 dark:text-white">Spending Trend</h3>
                <select 
                  value={timeRange}
         ...
         <div className="space-y-6 md:space-y-8">
           <div className="glass p-5 md:p-6 rounded-2xl flex flex-col h-[350px] md:h-[400px]">
              <div className="flex justify-between items-center mb-4 md:mb-6">
                <h3 className="text-lg font-semibold tracking-wide text-gray-900 dark:text-white">Recent</h3>
              </div>
              <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                 {recentTransactions.length === 0 ? (
                   <div className="flex flex-col items-center justify-center h-full text-gray-500 text-center">
                     ...
                     <p className="font-medium text-gray-900 dark:text-white mb-1">No transactions yet</p>
